...
let parsed_result =
  (* let filename = "matita_arithmetics_bigops.dk" in *)
  let filename = "test_file.dk" in
...
let main () =
  (* let filename = "matita_arithmetics_bigops.dk" in *)
  let filename, parsed = parsed_result in
      ...
      (fun state e ->
        let nw, data', rule_data, name_pragma =
          ...
          | Rules (_loc, partially_typed_rule_list) ->
              ...
              (* List.iter
                 (fun ({ name; ctx; pat; _ } : partially_typed_rule) ->
                   let rname = string_of pp_rule_name name in
                   let _param_type, _params =
                     List.fold_left
                       (fun (t, l) (_, ident, t') ->
                         assert (not (IdentSet.mem ident l));
                         ( t
                           |> IdentMap.add_option
                                (Option.map (fun t -> (ident, t)) t'),
                           l |> IdentSet.add ident ))
                       (IdentMap.empty, IdentSet.empty)
                       ctx
                   in

                   Printf.printf "Rule: %s %s ;; %s;; %s\n" (string_of_loc loc)
                     (string_of pp_pattern pat) rname
                     ((fun l ->
                        let s =
                          l
                          |> List.map (fun (_, ident, t) ->
                                 string_of_ident ident ^ ": "
                                 ^ (t
                                   |> Option.map (string_of pp_term)
                                   |> Option.value ~default:"UNKNOWN"))
                          |> String.concat ", "
                        in
                        s ^ ":: " ^ string_of_int (List.length l))
                        ctx);
                   let _ = process_pattern pat in
                   ())
                 partially_typed_rule_list; *)
              (None, [], data, None)
...
(* List.iter2 (fun (i1, ti) (i2, term_info) -> ()) types terms *)
(* locs |> LocMap.bindings
   |> List.iter (fun (loc, ident) ->
          Printf.printf "%s %s\n" (string_of_loc loc) (string_of_ident ident)); *)
(* List.iter
     (fun entry ->
       match entry with
       | Decl (_loc, ident, _scope, _staticity, _term) ->
           Printf.printf "%s %s\n" "Reading decl:" (string_of_ident ident)
       | Def (_loc, ident, _sign, _is_opaque, _t1, _t2) ->
           Printf.printf "%s %s %s \n" "Reading def:" (string_of_ident ident)
             (match _t1 with Some _t -> "Term" | None -> "None")
       | Rules (_loc, _partially_typed_rule_list) ->
           Printf.printf "%s %d\n" "Reading rules"
             (List.length _partially_typed_rule_list)
       | Name (_loc, _mident) ->
           Printf.printf "%s: %s \n" "Reading name" (string_of_mident _mident);
           (* let _ = 1 / 0 in *)
           ()
           (* (match _mident with Some _mident -> "Some" | None -> "None") *)
       | _ -> ())
     parsed;
   () *)
